...
test('check payload', async () => {
	const evA = jest.fn();
	const checkpoint = jest.fn(() => ({srcA: {evA}}));
	const write = jest.fn();
	const createWriter = jest.fn(() => ({write}));
	const sink = () => {};
	const source = await sourceFactory({db: {createWriter}, sink, opts: {checkpoint}});
	expect(checkpoint.mock.calls[0][0].sink).toBe(sink);
