...
test('check payload with checkpoint factory', async () => {
	...
	const close = jest.fn();
	const sink = jest.fn(() => Promise.resolve({close}));
	...
	checkpoint.mock.calls[0][0].registerCloseCallback(onClose);
...
test('wait for checkpoint factory to finish before any source may be used', async () => {
	let resolveCheckpoint;
	const checkpoint = jest.fn(() => new Promise((resolve) => { resolveCheckpoint = resolve; }));
	const createWriter = jest.fn(() => ({write: () => {}, close: () => {}}));
	const source = await sourceFactory({db: {createWriter}, opts: {checkpoint, rejectUnspecifiedEvents: true}});
	const q = Promise.all([source('srcA'), source('srcA')]);
	resolveCheckpoint({srcA: {evA: (x) => x}});
	await q;
});
